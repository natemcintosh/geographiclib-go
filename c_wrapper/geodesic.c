...
static real remquox(real x, real y, int* n) {
  ...
  if (n) {
    real a = remainderx(x, 2 * y), b = remainderx(x, 4 * y),
         c = remainderx(x, 8 * y);
    *n = (a > z ? 1 : (a < z ? -1 : 0));
    *n += (b > a ? 2 : (b < a ? -2 : 0));
    *n += (c > b ? 4 : (c < b ? -4 : 0));
...
static real areareduceA(real area[],
                        real area0,
                        int crossings,
                        boolx reverse,
                        boolx sign);
static real areareduceB(real area,
                        real area0,
                        int crossings,
                        boolx reverse,
                        boolx sign);
...
real geod_genposition(const struct geod_geodesicline* l,
  ...
  unsigned outmask = (plat2 ? GEOD_LATITUDE : GEOD_NONE) |
  ...
  if (outmask & GEOD_DISTANCE)
    s12 =
        (flags & GEOD_ARCMODE) ? l->b * ((1 + l->A1m1) * sig12 + AB1) : s12_a12;
...
void geod_gensetdistance(struct geod_geodesicline* l,
                         unsigned flags,
                         real s13_a13) {
  (flags & GEOD_ARCMODE) ? geod_setarc(l, s13_a13)
...
void geod_position(const struct geod_geodesicline* l,
                   ...
                   real* plon2,
                   real* pazi2) {
  geod_genposition(l, FALSE, s12, plat2, plon2, pazi2, nullptr, nullptr,
...
static real geod_geninverse_int(const struct geod_geodesic* g,
  ...
  unsigned outmask = (ps12 ? GEOD_DISTANCE : GEOD_NONE) |
                     (pm12 ? GEOD_REDUCEDLENGTH : GEOD_NONE) |
                     (pM12 || pM21 ? GEOD_GEODESICSCALE : GEOD_NONE) |
                     (pS12 ? GEOD_AREA : GEOD_NONE);
...
void geod_inverseline(struct geod_geodesicline* l,
                      ...
                      unsigned caps) {
  real salp1, calp1,
      a12 = geod_geninverse_int(g, lat1, lon1, lat2, lon2, nullptr, &salp1,
                                &calp1, nullptr, nullptr, nullptr, nullptr,
                                nullptr, nullptr),
...
void geod_inverse(const struct geod_geodesic* g,
                  ...
                  real* pazi1,
                  real* pazi2) {
  geod_geninverse(g, lat1, lon1, lat2, lon2, ps12, pazi1, pazi2, nullptr,
                  nullptr, nullptr, nullptr);
}
...
real Lambda12(const struct geod_geodesic* g,
  ...
  if (diffp) {
    if (calp2 == 0)
      ...
      Lengths(g, eps, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1, cbet2,
              nullptr, &dlam12, nullptr, nullptr, nullptr, Ca);
...
void geod_polygon_addpoint(const struct geod_geodesic* g,
  ...
  if (p->num == 0) {
    ...
    geod_geninverse(g, p->lat, p->lon, lat, lon, &s12, nullptr, nullptr,
                    nullptr, nullptr, nullptr, p->polyline ? nullptr : &S12);
    accadd(p->P, s12);
    if (!p->polyline) {
      accadd(p->A, S12);
...
void geod_polygon_addedge(const struct geod_geodesic* g,
  ...
  if (p->num) { /* Do nothing is num is zero */
    ...
    geod_gendirect(g, p->lat, p->lon, azi, GEOD_LONG_UNROLL, s, &lat, &lon,
                   nullptr, nullptr, nullptr, nullptr, nullptr,
...
unsigned geod_polygon_compute(const struct geod_geodesic* g,
  ...
  geod_geninverse(g, p->lat, p->lon, p->lat0, p->lon0, &s12, nullptr, nullptr,
  ...
  if (pA)
    *pA = areareduceA(t, 4 * pi * g->c2,
                      p->crossings + transit(p->lon, p->lon0), reverse, sign);
  return p->num;
}
...
unsigned geod_polygon_testpoint(const struct geod_geodesic* g,
  ...
  for (i = 0; i < (p->polyline ? 1 : 2); ++i) {
    ...
    geod_geninverse(g, i == 0 ? p->lat : lat, i == 0 ? p->lon : lon,
                    i != 0 ? p->lat0 : lat, i != 0 ? p->lon0 : lon, &s12,
                    nullptr, nullptr, nullptr, nullptr, nullptr,
                    p->polyline ? nullptr : &S12);
    perimeter += s12;
    if (!p->polyline) {
...
unsigned geod_polygon_testedge(const struct geod_geodesic* g,
  ...
  crossings = p->crossings;
  {
    ...
    geod_gendirect(g, p->lat, p->lon, azi, GEOD_LONG_UNROLL, s, &lat, &lon,
                   nullptr, nullptr, nullptr, nullptr, nullptr, &S12);
    tempsum += S12;
    crossings += transitdirect(p->lon, lon);
    geod_geninverse(g, lat, lon, p->lat0, p->lon0, &s12, nullptr, nullptr,
                    nullptr, nullptr, nullptr, &S12);
    perimeter += s12;
    tempsum += S12;
